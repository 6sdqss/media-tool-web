...
def run_mode_drive(w, h, drive_service):
    ...
    if st.button("🚀 BẮT ĐẦU CHẠY", type="primary", use_container_width=True):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for i, url in enumerate(links):
                ...
                try:
                    if kind == "folder":
                        ...
                        if not success:
                            st.warning(f"⚠️ Bỏ qua tải '{drive_name}' do lỗi quyền truy cập hoặc bị Google chặn.")
                            shutil.rmtree(current_raw, ignore_errors=True)
                            continue # Bỏ qua link lỗi, chạy tiếp link sau
            ...
            # DÙ CÓ LỖI HAY KHÔNG, CHỈ CẦN >= 1 LINK THÀNH CÔNG LÀ NÉN FILE ZIP
            if successful_links > 0 or st.session_state.download_status == 'cancelled':
                ...
                # Hàm os đã được import, không còn lỗi NameError
                if os.path.exists(temp_path / "Drive_Images_Done.zip"):
                    with open(temp_path / "Drive_Images_Done.zip", "rb") as f:
                        st.download_button("📥 TẢI DỰ PHÒNG TOÀN BỘ ẢNH (FILE ZIP)", f, "Drive_Images_Done.zip", "application/zip", type="primary", use_container_width=True)
            else:
